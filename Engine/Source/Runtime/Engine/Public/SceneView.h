...
public:
	...
	//YJH Created 2020/4/4
	ENGINE_API void HackCBRProjectionJitter(const uint8 CBRIndex, const int Width) {
		float myJitter_X = (4.f / Width) * (0.5f * CBRIndex);
		ProjectionMatrix.M[2][0] += myJitter_X;
		InvProjectionMatrix = InvertProjectionMatrix(ProjectionMatrix);
		RecomputeDerivedMatrices();
