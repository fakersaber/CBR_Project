...
void FOpenGLDynamicRHI::InitializeGLTexture(FRHITexture* Texture, uint32 SizeX, const uint32 SizeY, const bool bCubeTexture, const bool bArrayTexture, const bool bIsExternal, const uint8 Format, const uint32 NumMips, const uint32 NumSamples, const uint32 ArraySize, const uint32 Flags, const FClearValueBinding& InClearValue, FResourceBulkDataInterface* BulkData)
{
	...
	bool const bEnableCBR = CVarCBR->GetValueOnRenderThread() && NumSamplesTileMem == 2;
	GLuint TextureID = 0;
	//if enable CBR, alloc multisample texture
	if (!TileMemDepth || bEnableCBR)
	{
	...
	if (bEnableCBR)
	{
		...
		if (!TileMemDepth){
			...
			glTexStorage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, 2, GLFormat.InternalFormat[bSRGB], SizeX, SizeY, false);
			VERIFY_GL(glTexStorage2DMultisample);
		}
		else {
			Target = GL_TEXTURE_2D_MULTISAMPLE;
			glTexStorage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, 2, FOpenGL::SupportsPackedDepthStencil() ? GL_DEPTH24_STENCIL8 : GL_DEPTH_COMPONENT24, SizeX, SizeY, false);
			VERIFY_GL(glTexStorage2DMultisample);
		}
#endif
	}
	else if (NumSamples == 1 && !TileMemDepth)
	{
