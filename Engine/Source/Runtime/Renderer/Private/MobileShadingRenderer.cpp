...
void FMobileSceneRenderer::Render(FRHICommandListImmediate& RHICmdList)
{
	...
	bool IsEnableCBR = CVarMobileCBR.GetValueOnRenderThread() == 1;
	//YJH End
	...
	else
	{
		...
		//No need to resolve colors if CBR is turned on
		SceneColorResolve = !bMobileMSAA || IsEnableCBR ? nullptr : SceneContext.GetSceneColorTexture();
		//No need to resolve colors if CBR is turned on
		ColorTargetAction = !bMobileMSAA || IsEnableCBR ? ERenderTargetActions::Clear_Store : ERenderTargetActions::Clear_Resolve;
	...
	if (bRequiresTranslucencyPass)
	{
		...
		//no need to resolve color if the CBR is endable
		ColorTargetAction = !bMobileMSAA || IsEnableCBR ? ERenderTargetActions::Load_Store : ERenderTargetActions::Load_Resolve;
