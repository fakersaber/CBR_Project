...
void FSceneRenderTargets::AllocateCheckerboardColorBuffer(FRHICommandList& RHICmdList) {

	if (CheckerBoardRenderTarget[0] &&
		CheckerBoardRenderTarget[0]->GetRenderTargetItem().TargetableTexture->HasClearValue() &&
		!(CheckerBoardRenderTarget[0]->GetRenderTargetItem().TargetableTexture->GetClearBinding() == DefaultColorClear))
	{
		const FLinearColor CurrentClearColor = CheckerBoardRenderTarget[0]->GetRenderTargetItem().TargetableTexture->GetClearBinding().GetClearColor();
		const FLinearColor NewClearColor = DefaultColorClear.GetClearColor();
		UE_LOG(LogRenderer, Log, TEXT("Releasing previous color target to switch default clear from: %f %f %f %f to: %f %f %f %f"),
			CurrentClearColor.R,
			CurrentClearColor.G,
			...
			NewClearColor.B,
			NewClearColor.A);
		CheckerBoardRenderTarget[0].SafeRelease();
	}


	if (CheckerBoardRenderTarget[1] &&
		CheckerBoardRenderTarget[1]->GetRenderTargetItem().TargetableTexture->HasClearValue() &&
		!(CheckerBoardRenderTarget[1]->GetRenderTargetItem().TargetableTexture->GetClearBinding() == DefaultColorClear))
	{
		const FLinearColor CurrentClearColor = CheckerBoardRenderTarget[1]->GetRenderTargetItem().TargetableTexture->GetClearBinding().GetClearColor();
		const FLinearColor NewClearColor = DefaultColorClear.GetClearColor();
		UE_LOG(LogRenderer, Log, TEXT("Releasing previous color target to switch default clear from: %f %f %f %f to: %f %f %f %f"),
			CurrentClearColor.R,
			CurrentClearColor.G,
			...
			NewClearColor.B,
			NewClearColor.A);
		CheckerBoardRenderTarget[1].SafeRelease();
	}
	...
	//半分辨率
	FIntPoint DownSizeBuffer;
	QuantizeSceneBufferSize(FIntPoint(FMath::CeilToInt(BufferSize.X * 0.5f), FMath::CeilToInt(BufferSize.Y * 0.5f)), DownSizeBuffer);

	// Create the scene color.
	FPooledRenderTargetDesc Desc(FPooledRenderTargetDesc::Create2DDesc(DownSizeBuffer, SceneColorBufferFormat, DefaultColorClear, TexCreate_None, TexCreate_RenderTargetable | TexCreate_ShaderResource, false));

	//配置使用与SceneColor相同
	Desc.Flags |= GFastVRamConfig.SceneColor;

	// CBR NumSamples always 2
	Desc.NumSamples = 2;

	Desc.Flags |= MobileHWsRGB ? TexCreate_SRGB : 0;


	//创建两份RenderTarget,因为如果使用scenecolor作为原输入BufferSize太大
	if (!CheckerBoardRenderTarget[0]) {
		GRenderTargetPool.FindFreeElement(RHICmdList, Desc, CheckerBoardRenderTarget[0], TEXT("CBREvenRenderTarget"));
	}
	
	if (!CheckerBoardRenderTarget[1]) {
		GRenderTargetPool.FindFreeElement(RHICmdList, Desc, CheckerBoardRenderTarget[1], TEXT("CBROddRenderTarget"));
	}
	
	check(CheckerBoardRenderTarget[0]);
	check(CheckerBoardRenderTarget[1]);
}
...
void FSceneRenderTargets::AllocateMobileRenderTargets(FRHICommandList& RHICmdList)
{
	...
	//YJH Created 2020/3/14
	AllocateCheckerboardColorBuffer(RHICmdList);
	AllocateChckerboardDepthBuffer(RHICmdList);
	//YJH End
...
void FSceneRenderTargets::AllocateChckerboardDepthBuffer(FRHICommandList& RHICmdList) {
	// For mono support. We change the default depth clear value to the mono clip plane to clip the stereo portion of the frustum.
	if (CheckerBoardDepth[0] && !(CheckerBoardDepth[0]->GetRenderTargetItem().TargetableTexture->GetClearBinding() == DefaultDepthClear)) {
		CheckerBoardDepth[0].SafeRelease();
	}

	if (CheckerBoardDepth[1] && !(CheckerBoardDepth[1]->GetRenderTargetItem().TargetableTexture->GetClearBinding() == DefaultDepthClear)) {
		CheckerBoardDepth[1].SafeRelease();
	}

	//半分辨率
	FIntPoint CheckerBoardSize;
	QuantizeSceneBufferSize(FIntPoint(FMath::CeilToInt(BufferSize.X * 0.5f), FMath::CeilToInt(BufferSize.Y * 0.5f)), CheckerBoardSize);

	if (!CheckerBoardDepth[0])
	{
		// Using the result of GetDepthFormat() without stencil due to packed depth-stencil not working in array frame buffers.
		FPooledRenderTargetDesc Desc(FPooledRenderTargetDesc::Create2DDesc(CheckerBoardSize, PF_D24, DefaultDepthClear, TexCreate_None, TexCreate_DepthStencilTargetable | TexCreate_ShaderResource | TexCreate_InputAttachmentRead, false));
		Desc.Flags |= GFastVRamConfig.SceneDepth;
		Desc.NumSamples = 2;
		GRenderTargetPool.FindFreeElement(RHICmdList, Desc, CheckerBoardDepth[0], TEXT("CheckerBoardEvenSceneDepth"));
	}

	if (!CheckerBoardDepth[1])
	{
		// Using the result of GetDepthFormat() without stencil due to packed depth-stencil not working in array frame buffers.
		FPooledRenderTargetDesc Desc(FPooledRenderTargetDesc::Create2DDesc(CheckerBoardSize, PF_D24, DefaultDepthClear, TexCreate_None, TexCreate_DepthStencilTargetable | TexCreate_ShaderResource | TexCreate_InputAttachmentRead, false));
		Desc.Flags |= GFastVRamConfig.SceneDepth;
		Desc.NumSamples = 2;
		GRenderTargetPool.FindFreeElement(RHICmdList, Desc, CheckerBoardDepth[1], TEXT("CheckerBoardOddSceneDepth"));
	}

	check(CheckerBoardDepth[0]);
	check(CheckerBoardDepth[1]);
}
...
void FSceneRenderTargets::ReleaseAllTargets()
{
	...
	//YJH Created 2020/3/14
	for (int32 i = 0; i < UE_ARRAY_COUNT(CheckerBoardRenderTarget); i++)
	{
		CheckerBoardRenderTarget[i].SafeRelease();
	}

	for (int i = 0; i < UE_ARRAY_COUNT(CheckerBoardDepth); i++) {
		CheckerBoardDepth[i].SafeRelease();
	}
