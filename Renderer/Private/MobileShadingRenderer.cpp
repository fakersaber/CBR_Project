...
static TAutoConsoleVariable<int32> CVarMobileCBR(
	TEXT("r.MobileCBR"),
	1,
	TEXT("Enable CBR:\n"),
...
void FMobileSceneRenderer::InitViews(FRHICommandListImmediate& RHICmdList)
{
	...
	//YJH Created 2020/3/18
	if (CVarMobileCBR.GetValueOnRenderThread()) {
...
void FMobileSceneRenderer::Render(FRHICommandListImmediate& RHICmdList)
{
	...
	bool bKeepDepthContent = bForceDepthResolve || (bRenderToSceneColor && (bSeparateTranslucencyActive || View.bIsReflectionCapture ||
	...
	//YJH Created 2020/3/15
	//保存原始SecneColor与SceneDepth
	TRefCountPtr<IPooledRenderTarget> OriginSceneColorRT = SceneContext.GetSceneColor().GetReference();
	TRefCountPtr<IPooledRenderTarget> OriginSceneDepthZ = SceneContext.SceneDepthZ.GetReference();
	//YJH End
	...
	else
	{
		//YJH Created 2020/3/15
		if (CVarMobileCBR.GetValueOnRenderThread()) {
			SceneContext.SetSceneColor(SceneContext.CheckerBoardRenderTarget[SceneContext.CBRIndex]);
			SceneContext.SceneDepthZ = SceneContext.CheckerBoardDepth[SceneContext.CBRIndex];
			bMobileMSAA = true;
			ColorTargetAction = ERenderTargetActions::Clear_Resolve; //第一个Pass必须要Resolve,如果分Pass走半透明后面会自动设置Store
	...
	//YJH Created 2020/3/15
	FIntRect OriginViewSize = View.ViewRect;
	if (CVarMobileCBR.GetValueOnRenderThread()) {
		FIntPoint ViewSize = ApplyResolutionFraction(ViewFamily, View.ViewRect.Size(), 0.5f);
		FIntPoint QuantizeMinViewRect;
		QuantizeSceneBufferSize(FIntPoint(FMath::CeilToInt(View.ViewRect.Min.X * 0.5f), FMath::CeilToInt(View.ViewRect.Min.Y * 0.5f)), QuantizeMinViewRect);
		View.ViewRect.Min = QuantizeMinViewRect;
		View.ViewRect.Max = QuantizeMinViewRect + ViewSize;
	}
	//YJH End
	...
	if (bRequiresTranslucencyPass)
	{
		...
		//YJH Created 2020/3/15
		FIntPoint OriginFamilySize = FamilySize;
		if (CVarMobileCBR.GetValueOnRenderThread()) {
			FIntPoint FamiliyDownSize;
			QuantizeSceneBufferSize(FIntPoint(FMath::CeilToInt(FamilySize.X * 0.5f), FMath::CeilToInt(FamilySize.Y * 0.5f)), FamiliyDownSize);
			FamilySize = FamiliyDownSize;
		}
		//YJH End
	...
	//YJH Created 2020/3/15
	if (CVarMobileCBR.GetValueOnRenderThread()) {
		SceneContext.SetSceneColor(OriginSceneColorRT.GetReference());
		SceneContext.SceneDepthZ = OriginSceneDepthZ.GetReference();

		RHICmdList.BeginComputePass(TEXT("Reconstrct checkerboard"));

		uint32 flag = 0ul;
		flag |= 0x08; //render_checker_pattern_odd
		//flag |= 0x10; //render_checker_pattern_even

		RenderCBR_ColoResolve(
			RHICmdList,
			View.GetFeatureLevel(),
			View,
			static_cast<float>(SceneContext.CBRIndex),  //当前帧offset
			512.f,
			flag,
		...
		//RenderCBR_DepthResolve(
		//	RHICmdList,
		//	View.GetFeatureLevel(),
		//)

		RHICmdList.EndComputePass();
		SceneContext.CBRIndex = (SceneContext.CBRIndex + 1) % 2;
	}
	//YJH End
	...
	if (!bGammaSpace || bRenderToSceneColor)
	{
		...
		RHICmdList.TransitionResource(EResourceTransitionAccess::EReadable, OriginSceneColorRT->GetRenderTargetItem().TargetableTexture);
	}
...
//
//class FCopyMobileViewSceneColor_MSAA_PS : public FGlobalShader
//{
//	DECLARE_SHADER_TYPE(FCopyMobileViewSceneColor_MSAA_PS, Global);
//public:
//	static bool ShouldCompilePermutation(const FGlobalShaderPermutationParameters& Parameters)
//	{
//		return true;
//	}
//
//	FCopyMobileViewSceneColor_MSAA_PS() {}
//
//	void SetParameters(FRHICommandList& RHIcmdList, FTextureRHIRef SceneColor) {
//		FRHIPixelShader* ShaderRHI = GetPixelShader();
//		//FGlobalShader::SetParameters<FViewUniformShaderParameters>(RHIcmdList, ShaderRHI, ViewUniformBuffer);
//		SetTextureParameter(RHIcmdList, ShaderRHI, SceneColorTexture, SceneColorTextureSampler, TStaticSamplerState<SF_Bilinear>::GetRHI(), SceneColor);
//	}
//
//	virtual bool Serialize(FArchive& Ar) override {
//		const bool bShaderHasOutdatedParameters = FGlobalShader::Serialize(Ar);
//		Ar << SceneColorTexture;
//		Ar << SceneColorTextureSampler;
//		return bShaderHasOutdatedParameters;
//	}
//public:
//	FCopyMobileViewSceneColor_MSAA_PS(const ShaderMetaType::CompiledShaderInitializerType& Initializer)
//		: FGlobalShader(Initializer)
//	{
...
//};
//
//IMPLEMENT_SHADER_TYPE(, FCopyMobileViewSceneColor_MSAA_PS, TEXT("/Engine/Private/MobileViewMsaa.usf"), TEXT("MainPS"), SF_Pixel);
//
//void FMobileSceneRenderer::CopyMobileViewSceneColorMsaa(FRHICommandListImmediate& RHICmdList) {
//
//}
